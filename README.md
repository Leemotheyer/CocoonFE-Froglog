...
2. Enable install from unknown sources, then install the APK (`adb install -r cocoon-froglog-1.0.0.apk`). **Stock Cocoon can stay installed** — this build uses package `rip.moth.cocoonshell.froglog` and shows as **Cocoon (Froglog)** on the launcher.
